...
datos1 %>% 
  select(idencuesta, ola, just_pension, moblm, wlow_orig1) %>% 
  na.omit() %>% 
  mutate(just_pension = as.numeric(just_pension)) %>% 
  lm_robust(just_pension ~ moblm,
            weights = wlow_orig1,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% 
...
datos1 %>% 
  select(idencuesta, ola, just_pension, moblh, wlow_orig2) %>% 
  na.omit() %>% 
  mutate(just_pension = as.numeric(just_pension)) %>% 
  lm_robust(just_pension ~ moblh,
            weights = wlow_orig2,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos2 %>% 
  select(idencuesta, ola, just_pension, mobmh, wm_mh) %>% 
  na.omit() %>% 
  mutate(just_pension = as.numeric(just_pension)) %>% 
  lm_robust(just_pension ~ mobmh,
            weights = wm_mh,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos2 %>% 
  select(ola, idencuesta, just_pension, mobml, wm_ml) %>% 
  na.omit() %>% 
  mutate(just_pension = as.numeric(just_pension)) %>% 
  lm_robust(just_pension ~ mobml,
            weights = wm_ml,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos3 %>% 
  select(ola, idencuesta, just_pension, mobhm, wh_hm) %>% 
  na.omit() %>% 
  mutate(just_pension = as.numeric(just_pension)) %>% 
  lm_robust(just_pension ~ mobhm,
            weights = wh_hm,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos3 %>% 
  select(ola, idencuesta, just_pension, mobhl, wh_hl) %>% 
  na.omit() %>% 
  mutate(just_pension = as.numeric(just_pension)) %>% 
  lm_robust(just_pension ~ mobhl,
            weights = wh_hl,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos1 %>% 
  select(ola, idencuesta, just_health, moblm, wlow_orig1) %>% 
  na.omit() %>% 
  mutate(just_health = as.numeric(just_health)) %>% 
  lm_robust(just_health ~ moblm,
            weights = wlow_orig1,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos1 %>% 
  select(idencuesta, ola, just_health, moblh, wlow_orig2) %>% 
  na.omit() %>% 
  mutate(just_health = as.numeric(just_health)) %>% 
  lm_robust(just_health ~ moblh,
            weights = wlow_orig2,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()  
...
datos2 %>% 
  select(idencuesta, ola, just_health, mobmh, wm_mh) %>% 
  na.omit() %>% 
  mutate(just_health = as.numeric(just_health)) %>% 
  lm_robust(just_health ~ mobmh,
            weights = wm_mh,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos2 %>% 
  select(idencuesta, ola, just_health, mobml, wm_ml) %>% 
  na.omit() %>% 
  mutate(just_health = as.numeric(just_health)) %>% 
  lm_robust(just_health ~ mobml,
            weights = wm_ml,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos3 %>% 
  select(idencuesta, ola, just_health, mobhm, wh_hm) %>% 
  na.omit() %>% 
  mutate(just_health = as.numeric(just_health)) %>% 
  lm_robust(just_health ~ mobhm,
            weights = wh_hm,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos3 %>% 
  select(idencuesta, ola, just_health, mobhl, wh_hl) %>% 
  na.omit() %>% 
  mutate(just_health = as.numeric(just_health)) %>% 
  lm_robust(just_health ~ mobhl,
            weights = wh_hl,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos1 %>% 
  select(idencuesta, ola, just_educ, moblm, wlow_orig1) %>% 
  na.omit() %>% 
  mutate(just_educ = as.numeric(just_educ)) %>% 
  lm_robust(just_educ ~ moblm,
            weights = wlow_orig1,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos1 %>% 
  select(idencuesta, ola, just_educ, moblh, wlow_orig2) %>% 
  na.omit() %>% 
  mutate(just_educ = as.numeric(just_educ)) %>% 
  lm_robust(just_educ ~ moblh,
            weights = wlow_orig2,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg() 
...
datos2 %>% 
  select(idencuesta, ola, just_educ, mobmh, wm_mh) %>% 
  na.omit() %>% 
  mutate(just_educ = as.numeric(just_educ)) %>% 
  lm_robust(just_educ ~ mobmh,
            weights = wm_mh,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos2 %>% 
  select(idencuesta, ola, just_educ, mobml, wm_ml) %>% 
  na.omit() %>% 
  mutate(just_educ = as.numeric(just_educ)) %>% 
  lm_robust(just_educ ~ mobml,
            weights = wm_ml,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos3 %>% 
  select(idencuesta, ola, just_educ, mobhm, wh_hm) %>% 
  na.omit() %>% 
  mutate(just_educ = as.numeric(just_educ)) %>% 
  lm_robust(just_educ ~ mobhm,
            weights = wh_hm,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
...
datos3 %>% 
  select(idencuesta, ola, just_educ, mobhl, wh_hl) %>% 
  na.omit() %>% 
  mutate(just_educ = as.numeric(just_educ)) %>% 
  lm_robust(just_educ ~ mobhl,
            weights = wh_hl,
            se_type = "CR2",
            clusters = idencuesta,
            data = .) %>% screenreg()
