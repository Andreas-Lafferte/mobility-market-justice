...
# ess
sjmisc::frq(elsoc_long_2016_2023$ess_d)
sjmisc::frq(elsoc_long_2016_2023$ess_o)

elsoc_long_2016_2023$ess_d <- as.numeric(elsoc_long_2016_2023$ess_d)
elsoc_long_2016_2023$ess_o <- as.numeric(elsoc_long_2016_2023$ess_o)

elsoc_long_2016_2023$mobility <- (elsoc_long_2016_2023$ess_d - elsoc_long_2016_2023$ess_o)

sjmisc::frq(elsoc_long_2016_2023$mobility)

elsoc_long_2016_2023 <- elsoc_long_2016_2023 %>% 
  rowwise() %>% 
  mutate(mobility_f = case_when(mobility <= -1 ~ "Donward",
                                mobility == 0 ~ "Non-mobile",
                                mobility >= 1 ~ "Upward",
                                TRUE ~ NA_character_),
         mobility_f = factor(mobility_f,
                             levels = c("Non-mobile", "Donward", "Upward"))) %>% 
  ungroup()

frq(elsoc_long_2016_2023$mobility_f)
...
df_study1_long <- df_study1_wide %>%
  pivot_longer(
    cols = -c(idencuesta, muestra),
    names_to = c(".value", "ola"),
    names_pattern = "^(comuna|comunacod|ponderador_long_total|segmento|estrato|just_educ|just_pension|just_health|mjp|merit_effort|merit_talent|mobility|mobility_f|educ|educyear|sex|age|ideo|nhogar1|m46_nhogar|m54|m30|m30b|m29_imp)_(\\d+)$"
  ) %>%
...
df_study1_long <- df_study1_wide %>%
  pivot_longer(
    ...
    names_pattern = paste0(
      "^(comuna|comunacod|ponderador_long_total|segmento|estrato|just_educ|",
      "just_pension|just_health|mjp|merit_effort|merit_talent|mobility|",
      "mobility_f|educ|educyear|sex|age|ideo|quintil|",
      "quintil1|ing_pc)_(\\d+)$"
    )
  ) %>%
  mutate(ola = as.integer(ola))
